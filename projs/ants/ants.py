...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            # place is container ant
            if self.can_contain(place.ant):
                self.store_ant(place.ant)
                place.ant = self
            # self is not container ant
    ...
    def nearest_bee(self):
        ...
        while current_place.entrance:
            if not current_place.is_hive:
                if self.lower_bound <= places <= self.upper_bound and len(current_place.bees) > 0:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 12
        super().action(gamestate) # takes actions like other ThrowerAnts do
    ...
    def throw_at(self, target):
        ...
        def slow_action(gamestate):
            if gamestate.time % 2 == 0:
                # takes regular Bee action when gamestate time is even
                Bee.action(target, gamestate)
            # otherwise do nothing

            self.slow_turns += 1
            if self.slow_turns == 5: 
                target.action = lambda x: Bee.action(target, x)
