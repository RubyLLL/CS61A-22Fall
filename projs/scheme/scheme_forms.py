...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        lambda_procedure = do_lambda_form(expr, env)
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    if expressions is nil:
        return '#t' if is_scheme_true(scheme_eval(expressions, env)) else '#f'
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        evals = eval_all(bindings.first.rest, env)
        vals = Pair(evals, vals)
