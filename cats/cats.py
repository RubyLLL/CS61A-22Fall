...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    # case one,typed word contained in word list, return that word
    if typed_word in word_list:
        return typed_word
    lowest_diff_score = 100
    most_similar_word = typed_word
    for w in word_list:
        diff = diff_function(typed_word, w, limit)
        if diff <= limit:
            if diff < lowest_diff_score:
                lowest_diff_score = diff
                most_similar_word = w
...
def time_per_word(words, times_per_player):
    ...
    for tpp in times_per_player:
        match['times'].append([tpp[i]-tpp[i-1] for i in range(1, len(tpp))])
    return match
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    results = [[] for _ in range(len(times))]
