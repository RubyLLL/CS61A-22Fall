class Mint:
    ...
    def create(self, coin):
        ...
        # then creates and returns an instance of that class stamped 
        # with the mint's year (which may be different from Mint.present_year if it has not been updated.)
...
def store_digits(n):
    ...
    return result
    """
    # I wish I know why this wouldn't work
    def delete_last(ll):
    return Link(ll.first, delete_last(ll.rest)) if ll.rest is not Link.empty else Link.empty

    def only_last(ll):
        prev, curr = ll.first, ll.rest
    ...
    def reverse_ll(ll):
        if ll.rest is Link.empty:
            return ll
        last = only_last(ll)
        front = delete_last(ll)
        return Link(last, reverse_ll(front))
    """
